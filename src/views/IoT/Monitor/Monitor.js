...
import CreateFeed from "./CreateMonitor";
import UpdateFeed from "./UpdateMonitor";
import {
    Card,
    CardBody,
    CardHeader,
...
function Feed() {
    ...
    // setting nama menu
    const headings = ['Nomor Seri', 'Kandang', 'Tipe IoT'];
    const columns = ['serialNumber', 'house', 'deviceType'];

    const objectName = 'IoT';
    const queryDelete = `mutation{
                              deleteDevice(_id : "${deleteDevice}"){
                                deleted
                              }
                            }
                        `;
    ...
    const fetchData = (keyword = '', limit_ = limit, skip_ = skip, callback) => {
        const q = `query{
                      devices(keyword : "${keyword}", limit : ${limit_}, skip : ${skip_}){
                        totalCount
                        devices{
                            _id
                            serialNumber
        ...
        AsyncFetch(q, (res) => {
            setLoading(false);
            setObjects(res.data.data.devices.devices);
            const page = Math.ceil(parseInt(res.data.data.devices.totalCount) / limit_);
            callback(page);
        })
    };
    ...
    const showUpdateDialog = (_id) => {
        ...
        AsyncFetch(q, (res) => {
            result.devices = res.data.data.device;
        });
        AsyncFetch(q2, (res) => {
